...
export let operations = {
  ...
  ws1_culling:           {active: false, ws: '1', type: 'sow', label: 'Падеж маток/Вынужд.убой '},
  ...
  ws3_sow_culling:       {active: false, ws: '3s', type: 'sow', label: 'Падеж/Вынужд.убой маток'},
  ...
  switch (op.oper_name) {
    ...
    case 'ws3_sow_culling':
      component = 
        <SowRowOperation operation={op}>
          <td><div className='op-location'>{op.location} </div></td>
          <td><div className='op-location'>{op.culling_type == 'vinuzhd' ? 'вынужд. убой' : 'падеж'} </div></td>
